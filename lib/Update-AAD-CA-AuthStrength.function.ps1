function Update-AAD-CA-AuthStrength {
    ...
    foreach ($tier in $AuthStrengthTiers) {
        ...
        if ($tier -eq 0 -and $Force) {
            Write-Output ''
            Write-Warning "[Tier $tier] Azure AD Conditional Access Authentication Strengths can NOT be forcably updated in unattended mode: -Force parameter is ignored"
        }
