    ...
    displayName   = @(
        ...
        (
            ...
            ($EntraCAAuthContexts[2].default.id -replace '\D') + `
                '-Tier2-Roles-Unsupported-Devices'
        )
    ) | Join-String -Separator $DisplayNameElementSeparator
    description   = "Block PIM role enablement for privileged roles that are assigned to the '$($EntraCAAuthContexts[2].default.displayName)' authentication context from any device, except from those that are explicitly whitelisted. DO NOT CHANGE MANUALLY!"
