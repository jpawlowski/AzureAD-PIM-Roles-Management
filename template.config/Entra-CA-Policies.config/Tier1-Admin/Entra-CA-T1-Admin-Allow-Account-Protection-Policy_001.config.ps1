@(
    @{
        ...
        displayName   = @(
            ...
            'Admin-Users-Allow-Require-' + $EntraCAAuthStrengths[1].account.displayName
        ) | Join-String -Separator $DisplayNameElementSeparator
        description   = "Require '$($EntraCAAuthStrengths[1].account.displayName)' authentication methods for users without active Tier 1 Roles. DO NOT CHANGE MANUALLY!"
        ...
        grantControls = @{
            ...
            AuthenticationStrength = @{
                Id = $EntraCAAuthStrengths[1].account.Id
            }
        }
    }
)
