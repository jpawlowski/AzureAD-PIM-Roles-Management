        ...
        displayName   = @(
            ...
            (
                ...
                ($EntraCAAuthContexts[1].scopable.id -replace '\D') + `
                    '-Tier1-Scopable-Roles-Unsupported-Devices'
            )
        ) | Join-String -Separator $DisplayNameElementSeparator
        description   = "Block PIM role enablement for privileged roles that are assigned to the '$($EntraCAAuthContexts[1].scopable.displayName)' authentication context from any device, except those that are explicitly whitelisted. DO NOT CHANGE MANUALLY!"
